...
$(function(){
	...
	for(var i=0; i<allUser.length; i++){
		li+='<li><span class="onlineuid">'+allUser[i]+'</span><span class="invite" onclick="invite(this)">邀请</span></li>';
	}
	$('#user ul').html(li);
	*/
	//开始下子事件
			...
			success:function(text){
				if(text=='1'){
					...
					for (var i=0; i<order.length; i++){
						...
						//延迟2秒,因为setTimeout是并发执行，实在没办法才使用ajax
						$.ajax({
							type:'post',
							url:'delay.php',
							async:false
